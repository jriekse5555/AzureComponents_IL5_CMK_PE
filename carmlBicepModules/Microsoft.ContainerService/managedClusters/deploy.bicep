...
//Param diskEncryptionSetID added to support CMK
@description('Required. In order to achieve IL-5 Customer Managed Keys (CMK) must be used with AKS')
...
//Changed parameter to allow for using central AKS private dns zone
@description('Optional. Will allow AKS to either create a Private DNS Zone in the Node Resource Group using system, not with null, or use an existing private dns zone with resource id')
param privateDNSZoneId string
//old: @description('Optional. If AKS will create a Private DNS Zone in the Node Resource Group.')
//old: param usePrivateDNSZone bool = false
...
resource managedCluster 'Microsoft.ContainerService/managedClusters@2022-03-02-preview' = {
  ...
  properties: {
    ...
    apiServerAccessProfile: {
      ...
      //Param privateDNSZone changed in our environment to allow for setting the A record on a central private dns zone
      privateDNSZone: privateDNSZoneId
      //previous line-> privateDNSZone: usePrivateDNSZone ? 'system' : ''
    }
    podIdentityProfile: {
